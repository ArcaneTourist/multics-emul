...
static int _fetch_acc(int (*fetch)(uint addr, t_uint64 *wordp), uint addr, char bufp[513]);
//static int fetch_acc(uint addr, char bufp[513])
//{ return _fetch_acc(fetch_word, addr, bufp); }
        ...
        for (int link = first_link; link <= last_link; link += 2) {
            ...
            if ((word0 & 077) == 043) {
                ...
                if (typ == 0 || typ > 6) {
                    ...
                    if (qu == 0)
                        *sname = 0;
                    else if (_fetch_acc(fetch_word, defs.wordno + qu, sname) != 0)
                        return 2;
                    if (ql == 0)
                        *ename = 0;
                    else if (_fetch_acc(fetch_word, defs.wordno + ql, ename) != 0)
                        return 2;
                    if (exp_offset == 0)
    ...
    int (*fetch)(uint addr, t_uint64 *wordp))
{
    ...
    for (;;) {
        ...
        uint namep = word2 >> 18;
        char buf[513];
        if (_fetch_acc(fetch, hdrp + namep, buf) != 0)
            return 2;
        if (def_class == 3) {
...
static int scan_seg_defs(uint segno, AR_PR_t *defsp, int name_only, int msgs)
{
    ...
    for (def_it = seg_defs.defs.begin(); def_it != seg_defs.defs.end(); ++ def_it) {
        ...
        if (msgs || (def.type == def_seg_name && name_only))
            out_msg("Def entry at %o|%04o (offset %04o): class %#o.  ", TPR.TSR, defp, defp - hdrp, def.type);
        switch(def.type) {
            case def_text:
                if (msgs)
